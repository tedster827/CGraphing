/*
Legal
Copyright 2016 Teddy Williams All rights reversed
Please ask permission to use or copy this code in anyway.
*/
...
slope evalSlope(string &equation) {
  // Instantiating slope for a return value of the function.
  // The default slope is set to 1 if digits aren't found before 'x'.
  slope equSlope = 1;
  ...
  if (equation[0] == 'y') {
    ...
    for (int i = 0; i < equation.size(); i++) {
      ...
      if (equation[i] == 'x') {
        // x found the slope can not be undefined the zSlope flag is set to false.
        zSlope = false;
        // Loop to search backwards through an equation, find digits in the slope.
        for (int j = i; j > 0; j--) {
          // Once it finds an '-' the slope is without a doubt negitive.
          if (equation[j] == '-') {
            sign = false;
            // Negative sign found slope negative the sign flag is set to false.
          }
          // Reguardless of spacing between x and the last digit it will only evaluate when it finds a digit, which has to be the last digit of the slope.
          if (isdigit(equation[j])) {
            ...
            while (isdigit(equation[j-1])) {
              // Decrementing backwards finding other digits of the slope.
              j--;
              ...
              if (isdigit(equation[j])) {
                ...
                cout << "Evaluation : ";
                cout << equation[j] - 48 << " * (10" << "^" << powerOfTen << ")" << endl;
